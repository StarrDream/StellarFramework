...
- HybridCLR 代码热更新产物需另行通过 ToolsHub 的 `HybridCLR DLL 导出` 生成；Addressables Player Content 可通过 `Addressables` 模块构建；YooAsset 内容版本/下载产物由项目内容管线负责。`BuildScript` 不替代这些资源发布步骤。
