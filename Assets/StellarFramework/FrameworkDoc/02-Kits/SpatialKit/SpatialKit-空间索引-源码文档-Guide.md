...
- Core 为纯 C# 数据结构，不要求 PlayMode 或 Integration Scene；行为正确性由 EditMode 测试直接验证公开 API。

## 7. 导出边界

Core Profile 的 source path 只有 `Assets/StellarFramework/Runtime/Kits/SpatialKit`，导出闭包不应包含其他 Kit、Samples、Tests、Verification 或 UPM。Catalog 不再提供 `samples.*` 分发 Profile。
