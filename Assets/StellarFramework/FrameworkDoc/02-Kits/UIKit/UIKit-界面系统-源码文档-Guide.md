...
屏幕适配源码已独立到 `Runtime/Kits/UIAdaptationKit`，不属于 UIKit.Core。
...
Optional UIAdaptationKit Integration
├─ FullScreenRoot
├─ SafeAreaRoot
└─ PanelLayoutRegion -> 对接独立 UIAdaptationKit
```
...
## 独立 `UIAdaptationKit` 集成

UIKit 不再拥有屏幕适配算法。需要 SafeArea / PreciseCutout / Breakpoint / LayoutVariant 时组合独立 `UIAdaptationKit`。

UIKit 的集成职责只有：

- 标准 UIRoot 提供 `FullScreenRoot / SafeAreaRoot`；
- `PanelLayoutRegion.SafeArea` 把 Panel 路由到 SafeAreaRoot；
- `UIKit Complete` 分发时组合 `UIAdaptationKit.Tools`。

UIAdaptationKit Runtime 可以在完全没有 UIKit 的项目中单独工作。算法、Fallback、Geometry 和完整源码说明见：

- `FrameworkDoc/02-Kits/UIAdaptationKit/UIAdaptationKit-源码文档-Guide.md`
...
- 栈顶全屏遮挡逻辑
- Snapshot 输出
- UIKit Core 不导入 UIAdaptationKit 时可独立编译/运行
- UIKit Complete 组合 UIAdaptationKit 后 SafeArea Panel 路由正常

## 相关文档

- [UIKit 说明文档](UIKit-界面系统-说明文档-Guide.md)
- [UIAdaptationKit 使用文档](../UIAdaptationKit/UIAdaptationKit-使用文档-Guide.md)
- [UIAdaptationKit 源码文档](../UIAdaptationKit/UIAdaptationKit-源码文档-Guide.md)
