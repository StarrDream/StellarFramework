...
> 状态：GridKit V1 Square Semantics Frozen / Topology Additive Extension

## Design Goals 与 Foundation 边界

源码优先保证数学正确性、确定性、固定内存布局和小而稳定的 API。GridKit 描述二维纯逻辑网格与拓扑；现有 DenseGrid/Footprint/Occupancy 仍保持正交 `GridCoord` 语义，Topology 能力额外提供 Square/Hex Topology。任何“这个格子为什么能放建筑”“Chunk 何时加载”“NPC 如何寻路”的业务语义都必须留在后续 Kit 或业务项目。

Core 没有隐藏初始化、Shutdown、Update、Driver、Singleton、Manager 或 Global Registry。构造 `new DenseGrid<T>(bounds)` 即可使用。

## 目录与程序集

`Assets/StellarFramework/Runtime/Kits/GridKit` 是唯一 Runtime 根目录，`StellarFramework.GridKit.Core.asmdef` 没有程序集引用并设置 `noEngineReferences=true`。源码按 `Geometry`、`Storage`、`Footprints`、`Occupancy`、`Topology` 分组；目录划分只服务框架开发工程，导出器会将该目录作为一个 Kit 闭包导出。
...
Hex 邻居顺序固定为 `East / NorthEast / NorthWest / West / SouthWest / SouthEast`。`HexEdge` 将两个相邻 Cell 按 `(Q,R)` canonical 排序；`HexVertex` 将共享顶点的三个 Cell canonical 排序，因此从不同相邻 Cell 查询同一 Edge/Vertex 时 identity 相等。

## Topology 实现

`IGridTopology<TCoord>` 只有最大邻居数、caller-owned `Span<TCoord>` 邻居写入和逻辑距离三个基础职责。它不包含存储、寻路、业务 Cost 或世界生命周期。

`Orthogonal4Topology` / `Orthogonal8Topology` 直接复用现有 `GridNeighbors` 与 `GridDistance`，因此旧 Square 语义没有复制第二套实现。

`HexTopology` 使用 Axial `HexCoord(Q,R)`：
...
- `WriteRing`；
- `WriteRange`；
- `HexEdge` / `HexVertex` capability。

Ring/Range 在写入前验证目标 Buffer 容量和 Int32 坐标半径，失败不留下 partial write。邻居在单个方向超出 Int32 时 `TryGetNeighbor=false`，checked API 抛 `OverflowException`。

`IGridEdgeTopology<TCell,TEdge>` 与 `IGridVertexTopology<TCell,TVertex>` 是 optional capability，不强迫每一种拓扑都伪造 Edge/Vertex。
...
IGridTopology<GridCoord> → Orthogonal4Topology / Orthogonal8Topology
IGridTopology<HexCoord> + Edge/Vertex capabilities → HexTopology
```
...
`GridTopologyTests.cs` 覆盖 Square Topology 兼容、Hex 负坐标、稳定邻居顺序、距离、Ring/Range、Int32 溢出、caller buffer、canonical Edge/Vertex shared identity。

`GridKitBenchmarkTests` 在 Unity Editor 中执行：1000×1000 DenseGrid（填充、线性 Span 读写、坐标索引读写）、1M Rect/坐标↔index 往返，以及 100k `CanOccupy`、`TryOccupy`/`TryRelease`；Topology 部分另执行 1M Orthogonal4/Orthogonal8/Hex 邻居+距离查询。输出 Unity 版本、各段 elapsed milliseconds、校验和和 `GC.GetTotalMemory` 变化；没有机器相关的固定性能门槛。
...
| Topology Neighbor/Distance | O(1) | caller-owned Span，目标 0 managed allocation |
| Hex Ring | O(radius) | caller-owned Span |
| Hex Range | O(radius²) | caller-owned Span |
...
1. GridKit.Core 只描述 2D 逻辑 Grid/Topology；Square storage/occupancy 与 Hex topology 的职责必须区分。
...
6. Rect、Square Neighbor、Hex Neighbor、Footprint 的公开顺序固定且可测试。
...
| Topology | Square 4/8 兼容、Hex 邻居/距离、Ring/Range、Edge/Vertex identity、溢出、容量 |
...
Editor Benchmark 使用 1000×1000（1,000,000）DenseGrid 执行 Fill、Span 线性读写和坐标读写；执行 1M Rect foreach 与 Coord↔Index round-trip；执行 100,000 次 `CanOccupy` 及 `TryOccupy`/`TryRelease`；Topology 基准增加 1M Square4/Square8/Hex6 neighbor+distance 查询。报告 elapsed、分配变化、Unity 版本和校验和，不设置固定毫秒门槛；目标平台性能由发布项目单独测量。

## 边界与演进

Pathfinding、Sparse/Chunk、3D、Tilemap、Placement、Save 和 Event 都必须在独立 Kit/Adapter 中设计，不得为了“方便”写进 Core。Hex Topology 已作为纯逻辑 GridKit 能力加入，但 Hex Storage/Occupancy/Pathfinding 仍必须按真实需求独立设计。若未来增加 `GridKit.Tools`，它应作为独立 ToolsHub 子模块和独立导出 Profile，不能反向引用 Runtime Core 以外的 Kit。
