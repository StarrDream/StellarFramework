...
namespace StellarFramework.Examples
{
    ...
    public class Example_ResKit : MonoBehaviour
    {
        ...
        {
            ReleaseRuntimeResources(reallocateLoaders: false);
        }
        ...
        private async UniTaskVoid TestAssetBundleAsync()
        {
            ...
            if (prefab == null)
            {
                prefab = AssetDatabase.LoadAssetAtPath<GameObject>(assetBundlePath);
                if (prefab != null)
                {
                    _statusText =
                        "AssetBundle 构建产物未命中，已回退到 Editor 直读源 Prefab。\n这说明场景接线已正常，但你仍应补齐正式 Bundle 产物再验证物理加载。";
                }
            }
#endif
            if (prefab != null)
            {
                ...
                if (!_statusText.Contains("回退到 Editor 直读"))
                {
 ...
 "Addressables 加载失败！\n请确认：\n1. 是否在 Groups 窗口点击了 Build？\n2. Play Mode Script 是否切换为了 Use Existing Build？";
        ...
        private void CleanupMemory()
        {
            ...
            ReleaseRuntimeResources(reallocateLoaders: true);
        ...
        private void ReleaseRuntimeResources(bool reallocateLoaders)
        {
            ...
            if (_resLoader != null)
            {
                ResKit.Recycle<ResourceLoader>((ResourceLoader)_resLoader);
                _resLoader = null;
            }

            if (_abLoader != null)
            {
                ResKit.Recycle<AssetBundleLoader>((AssetBundleLoader)_abLoader);
                _abLoader = null;
            }

            if (_customLoader != null)
            {
                ResKit.Recycle<RawTextLoader>((RawTextLoader)_customLoader);
                _customLoader = null;
            }
