...
*   **常规 C# event 的问题**：
    *   `+=` 和 `-=` 操作可能会产生委托合并的内存分配。
    *   如果 View 销毁时忘记 `-=`，Model 会一直持有 View 的引用，导致内存泄漏。

**BindableKit 的特性：**
*   **低 GC 设计**：内部使用双向链表与静态对象池管理观察者节点。减少运行时的内存分配。
*   **防忘注销**：提供 `UnRegisterWhenGameObjectDestroyed`，利用 Unity 的 Component 机制自动绑定生命周期。
*   **集合支持**：支持 `List` 和 `Dictionary` 的增删改查通知。
...
### 2.1 数据结构
`BindableProperty` 内部维护了一个 `ObserverNode` 的双向链表：
```csharp
class ObserverNode {
    Action<T> Action;
...
```
*   **注册**：从静态池取一个 Node，挂到链表尾部。
*   **注销**：将 Node 从链表中移除，放回静态池。

### 2.2 自动注销机制
当调用 `.UnRegisterWhenGameObjectDestroyed(gameObject)` 时，框架会在该 GameObject 上获取（或添加）一个隐藏组件 `EventUnregisterTrigger`。
当该 GameObject 被销毁时，触发所有注册在该 Trigger 上的注销逻辑。
...
public class PlayerUI : StellarView
{
    ...
    public override void OnBind()
    {
        ...
        // RegisterWithInitValue: 注册并立即执行一次回调
        // UnRegisterWhenGameObjectDestroyed: 绑定生命周期
...
**Service 层修改：**
```csharp
public class DamageService : AbstractService
{
...
### Q1: 为什么值变了没收到通知？
*   **机制**：`BindableProperty` 内部使用了 `EqualityComparer`。如果新值和旧值相等（例如 `HP` 从 100 变成 100），默认**不会**触发通知。
*   **解决**：如果需要强制通知（例如引用类型内容变了但引用没变），请使用 `.SetValueForceNotify(val)`。

### Q2: 列表操作 `items[i].xxx = yyy` 没反应？
*   **机制**：`BindableList` 只能监听列表本身的结构变化（Add/Remove/Replace）。如果修改了列表中某个对象的**内部字段**，列表是感知不到的。
*   **解决**：
    1.  替换整个对象：`list[i] = newObj;` (触发 Replace)。
    2.  或者对象内部属性也使用 `BindableProperty`。

### Q3: 闭包 GC 问题 (性能优化)
如果在 `Register` 时使用了**捕获变量的 Lambda 表达式**，C# 编译器会生成闭包类，产生额外 GC。
*   **建议做法**:
```csharp
