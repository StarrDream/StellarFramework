...
- `ResLoadBackend`：Resources、Addressables、AssetBundle、Custom 后端枚举。
...
- `AddressableLoader`：基于 Addressables 的后端。
- `AddressableHotUpdateManager`：AA catalog、hash、下载、缓存和清理管理。
...
Addressables 远端热更的数据流独立处理版本：`AddressableHotUpdateManager` 初始化 AA，检查 catalog/hash，更新 catalog，按 key 计算下载大小，下载依赖并让 AA 管缓存。ResKit 只负责把加载 API 统一给业务层。

## 依赖关系

- Resources 后端依赖 Unity Resources。
- AB 后端依赖 `Generated/AssetMap` 和 Unity AssetBundle API。
- AA 后端依赖 Addressables。
- HotUpdateKit 依赖 ResKitRuntimeSettings 和 AddressableHotUpdateManager。
- ToolsHub 的 AB、AA、ResKit 审计工具依赖 ResKit 数据结构。

## 扩展点

- 新增第三方后端：继承 `ResLoader`，实现真实同步/异步加载和卸载，注册 `ResKit.RegisterCustomLoader("YooAsset", request => new YooAssetResLoader())`。
