...
namespace StellarFramework
{
    ...
    public static class TweenKit
    {
        ...
        public static async UniTask To(float start, float end, float duration, Action<float> onUpdate, Ease ease,
            CancellationToken token, bool ignoreTimeScale, IProgress<float> progress = null)
        {
            if (duration <= 0f)
            {
                onUpdate?.Invoke(end);
        ...
        public static async UniTask To(Vector3 start, Vector3 end, float duration, Action<Vector3> onUpdate, Ease ease,
            CancellationToken token, bool ignoreTimeScale, IProgress<float> progress = null)
        {
            if (duration <= 0f)
            {
        ...
        public static async UniTask To(Color start, Color end, float duration, Action<Color> onUpdate, Ease ease,
            CancellationToken token, bool ignoreTimeScale, IProgress<float> progress = null)
        {
            if (duration <= 0f)
            {
    ...
    public static class TweenExtensions
    {
        ...
        public static UniActionChain MoveTo(this UniActionChain chain, Transform target, Vector3 endPos, float duration,
        ...
        public static UniActionChain LocalMoveTo(this UniActionChain chain, Transform target, Vector3 endPos,
        ...
        public static UniActionChain ScaleTo(this UniActionChain chain, Transform target, Vector3 endScale,
            float duration, Ease ease = Ease.OutBack)
        {
            chain.AppendTask(async (token) =>
            {
                if (target == null) return;
        ...
        public static UniActionChain ScaleTo(this UniActionChain chain, Transform target, float endScale,
            float duration, Ease ease = Ease.OutBack)
        {
            return ScaleTo(chain, target, Vector3.one * endScale, duration, ease);
        }

        public static UniActionChain RotateTo(this UniActionChain chain, Transform target, Vector3 endEuler,
            float duration, Ease ease = Ease.OutQuad)
        {
            chain.AppendTask(async (token) =>
            {
        ...
        public static UniActionChain FadeTo(this UniActionChain chain, CanvasGroup target, float endAlpha,
        ...
        public static UniActionChain FadeTo(this UniActionChain chain, Graphic target, float endAlpha, float duration,
        ...
        public static UniActionChain ColorTo(this UniActionChain chain, Graphic target, Color endColor, float duration,
            Ease ease = Ease.Linear)
        {
            chain.AppendTask(async (token) =>
            {
        ...
        public static UniActionChain ValueTo(this UniActionChain chain, float start, float end, float duration,
            Action<float> onUpdate, Ease ease = Ease.OutQuad)
        {
            chain.AppendTask(async (token) =>
            {
                await TweenKit.To(start, end, duration, onUpdate, ease, token, chain.IsIgnoreTimeScale);
            });
            return chain;
        }
