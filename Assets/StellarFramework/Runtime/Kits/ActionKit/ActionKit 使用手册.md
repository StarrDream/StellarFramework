...
### 基础序列
所有动作必须以 `ActionKit.Sequence(gameObject)` 开头，以 `.Start()` 结尾。
...
## 3. ActionKit：流程控制 (Flow Control)

ActionKit 提供了对时间、帧、条件和循环的控制能力。
...
*   **做法**：如果需要重新播放，必须**重新构建**序列（参考 5.1 的写法，每次都调用 `ActionKit.Sequence`）。
...
`ActionKit.Sequence(gameObject)` 传入的 `gameObject` 是绑定的生命周期对象。
...
3.  **报错 "Object is already recycled" 或空引用**
    ...
    *   解决：每次播放动画时，请重新调用 `ActionKit.Sequence(...)` 创建新的链条。
